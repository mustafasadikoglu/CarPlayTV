...
// Note: NSAllowsArbitraryLoadsForMedia and NSAllowsLocalNetworking must NOT be present,
// because if set to true, Apple ATS ignores NSAllowsArbitraryLoads for remote URLSession data tasks!
...
if (infoPlistContent.includes("<key>NSAllowsLocalNetworking</key>")) {
    throw new Error("Info.plist: NSAllowsLocalNetworking must NOT be present as it causes ATS to ignore NSAllowsArbitraryLoads for remote servers!");
}
console.log("- Unconditional NSAllowsArbitraryLoads verified (covers all remote HTTP/HTTPS data & media requests).");
