...
function parseSeriesDetails(payload, server, username, password, series) {
    ...
    if (payload.episodes && typeof payload.episodes === 'object') {
        for (const [seasonKey, epList] of Object.entries(payload.episodes)) {
            ...
            vodEpisodes.sort((a, b) => a.episodeNumber - b.episodeNumber);
            seasonsDict[sNum] = vodEpisodes;
        }
    }

    const seasons = Object.keys(seasonsDict).sort((a, b) => Number(a) - Number(b)).map(sNum => ({
...
assert.strictEqual(ep1.streamURL, 'http://iptv.server.org:8080/series/testuser%40iptv/pass%23secret%2F123/50101.m3u8');
...
assert.strictEqual(altResult.seasons[0].episodes[0].streamURL.endsWith('991.m3u8'), true, 'mkv mapped to m3u8 for AVPlayer');
